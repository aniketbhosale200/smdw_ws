...
TEST_F(MecanumDriveControllerTest, when_controller_is_configured_expect_all_parameters_set)
{
  ...
  ASSERT_EQ(controller_->params_.kinematics.circular_radius, 0.0);
  ASSERT_EQ(controller_->params_.kinematics.semi_major_axis, 0.0);
  ASSERT_EQ(controller_->params_.kinematics.semi_minor_axis, 0.0);
  ...
  ASSERT_EQ(controller_->params_.kinematics.circular_radius, 0.5);
  ASSERT_EQ(controller_->params_.kinematics.semi_major_axis, 0.55);
  ASSERT_EQ(controller_->params_.kinematics.semi_minor_axis, 0.45);
...
TEST_F(MecanumDriveControllerTest, when_controller_configured_expect_properly_exported_interfaces)
{
  ...
  for (size_t i = 0; i < state_intefaces.names.size(); i += 2)
  {
    EXPECT_EQ(state_intefaces.names[i], command_joint_names_[i / 2] + "/" + interface_name_);
    EXPECT_EQ(state_intefaces.names[i + 1], command_joint_names_[i / 2] + "/position");
  }
  ...
  when_reference_msg_received_expect_updated_commands_and_status_message)
{
  ...
  // With position = 0.1 rad, effective radius = circular_radius (0.5)
  // joint_command_values_[REAR_LEFT] = (1.5 + 0.0 - 1.0 * 0.0) / 0.5 = 3.0
  EXPECT_EQ(joint_command_values_[controller_->get_rear_left_wheel_index()], 3.0);

  subscribe_to_controller_status_execute_update_and_get_messages(msg);

  ASSERT_EQ(msg.reference_velocity.linear.x, 1.5);
  ASSERT_EQ(msg.back_left_wheel_velocity, 3.0);  // Updated due to dynamic radius
  ASSERT_EQ(msg.back_right_wheel_velocity, 3.0); // Updated due to dynamic radius
}
...
TEST_F(MecanumDriveControllerTest, SideToSideAndRotationOdometryTest)
{
  ...
  for (double t = 0; t < test_duration; t += dt)
  {
    ...
    joint_state_values_[fl_index * 2] = controller_->command_interfaces_[fl_index].get_value();
    joint_state_values_[fr_index * 2] = controller_->command_interfaces_[fr_index].get_value();
    joint_state_values_[rl_index * 2] = controller_->command_interfaces_[rl_index].get_value();
    joint_state_values_[rr_index * 2] = controller_->command_interfaces_[rr_index].get_value();
    // Increment positions (simulating wheel rotation)
    joint_state_values_[fl_index * 2 + 1] += joint_state_values_[fl_index * 2] * dt;
    joint_state_values_[fr_index * 2 + 1] += joint_state_values_[fr_index * 2] * dt;
    joint_state_values_[rl_index * 2 + 1] += joint_state_values_[rl_index * 2] * dt;
    joint_state_values_[rr_index * 2 + 1] += joint_state_values_[rr_index * 2] * dt;
  }
